...
# using tanh over logistic sigmoid is recommended    / tan_hyperbolique
def tanh(x):
    return math.tanh(x)
...
class MLP_NeuralNetwork(object):
    ...
    def predict(self, patterns):
        ...
        """
        #for p in patterns:
        #print(patterns[1], '->', self.feedForward(patterns[0]))
        return self.feedForward(patterns)
    ...
    def test(self, patterns,entry):
        ...
        for p in patterns:
            ...
            if result[0] != p[1][0] or result[1] != p[1][1] or result[2] != p[1][2]:
                cpt=cpt+1
        entry.insert(END,"le taux d'ereur est : {} \n".format((float(cpt)/float(len(patterns)))))
        #return (predictions,float(cpt)/float(len(patterns)))
        '''predictions = []
        for p in X:
